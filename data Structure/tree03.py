#1 ~ 1000 숫자 중 임의의 100개 수를 트리에 입력 검색 삭제 테스트
class Node:
...
head = Node(500)
bn_tree = NoneMgmt(head)
for num in bst_nums:
    bn_tree.insert(num)

for num in bst_nums:
    if bn_tree.search(num) == False:
        print("fail",num)
...
for denum in del_nums:
    if bn_tree.delete(denum) == False:
        print("delete Fail",denum)